...
pj_bool_t onRxRequest(pjsip_rx_data *rdata)
{
    ...
    if (msg->line.req.method.id == PJSIP_OTHER_METHOD)
    {
        ...
        if (rootType == string(SIP_NOTIFY) && cmdValue == string(SIP_HEARTBEAT))
        {
    ...
    // std::shared_ptr<SipTaskBase> reg=make_shared<SipRegister>();
    /* SipTaskBase* reg=new SipRegister();
    reg->run(rdata); */
    // pjsip 内部有个消息队列 单个线程处理消息，处理不及时容易将队列填满
    pthread_t pid;
